<link rel="stylesheet" href="../public/css/theory_select.css">
<br>
<div class="container course fst-italic" style="margin-top:0px;">
    <a href="../public/theory_exercise.php">
        <div class="back text-light">
            <h5>
                << Trở về</h5>
        </div>
    </a>
    <div class="container">
        <div class="row">
            <div class="col-1">
            </div>
            <div class="col-10">
                <div class="text-center text-light">
                    <h1><strong>Bài tập âm nhạc dành cho bạn !!!</strong></h1>

                </div><br><br>
            </div>
            <div class="col-1">
            </div>
        </div>
    </div>
    <div class="box">
        <a href="../public/theory_selected.php?loai=note&capdo=<?php echo $_GET['capdo'] ?>">
            <div class="content">
                <h2>Nốt nhạc</h2>
                <p>Là những nốt cơ bản nhất, cấu tạo nên nhạc</p>
            </div>
            <div class="image">
                <img src="../library/image/khoahoc/4_1.png">
            </div>
            <br>
        </a>
    </div>
    <div class="box">
        <a href="../public/theory_selected.php?loai=chord&capdo=<?php echo $_GET['capdo'] ?>">
            <div class="content">
                <h2>Hợp âm</h2>
                <p>Là tập hợp của nhiều nốt nhạc</p>
            </div>
            <div class="image">
                <img src="../library/image/khoahoc/4_2.png">
            </div>
            <br>
        </a>
    </div>
    <div class="box">
        <a href="../public/theory_selected.php?loai=melody&capdo=<?php echo $_GET['capdo'] ?>">
            <div class="content">
                <h2>Nhịp diệu</h2>
                <p>Sự kết hợp của các hợp âm theo qui luật</p>
            </div>
            <div class="image">
                <img src="../library/image/khoahoc/4_3.png">
            </div>
            <br>
        </a>
    </div>

</div>
</div><br><br><br><br><br><br><br>
