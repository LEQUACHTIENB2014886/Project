!(function () {
    ...
    e.prototype = {
      ...
      volume: function (e) {
        var o = this || n;
        if (
          ((e = parseFloat(e)), o.ctx || _(), void 0 !== e && e >= 0 && 1 >= e)
        ) {
      ...
      _autoResume: function () {
        var e = this;
        return e.ctx && void 0 !== e.ctx.resume && n.usingWebAudio
    ...
    o.prototype = {
      ...
      _startFadeInterval: function (e, n, o, t, r) {
        var a = this,
          ...
          _ = d / 0.01,
          s = _ > 0 ? t / _ : t;
        4 > s && ((_ = Math.ceil(_ / (4 / s))), (s = 4)),
          (e._interval = setInterval(function () {
            _ > 0 && (u += "in" === i ? 0.01 : -0.01),
              (u = Math.max(0, u)),
              (u = Math.min(1, u)),
              (u = Math.round(100 * u) / 100),
              a._webAudio
                ? (void 0 === r && (a._volume = u), (e._volume = u))
                : a.volume(u, e._id, !0),
              ((n > o && o >= u) || (o > n && u >= o)) &&
    ...
    (function () {
      function isNotesMarked() {
        collectAllNotes[0] &&
          (Object.values(collectAllNotes[0]).length >= 1
            ? ($(".btn-reset").removeClass("hidden"),
              $(".playAll").removeClass("hidden"))
            : ($(".btn-reset").addClass("hidden"),
              $(".playAll").addClass("hidden")));
      }
      function markNotes(key) {
        if (!markTrigger) {
          var currentKey = key.attr("data-note");
          lockEvent[currentKey] ||
            key
              .toggleClass("marked")
              .promise()
              .done(function () {
                collectAllNotes.length > 0 && (tempObjNote = collectAllNotes[0]),
                  $(this).hasClass("marked")
                    ? ((tempObjNote[currentKey] = currentKey),
                      (collectAllNotes[0] = tempObjNote),
                      (collectNotesToArr = Object.values(tempObjNote)))
                    : (delete collectAllNotes[0][currentKey],
                      (collectNotesToArr = Object.values(tempObjNote)));
              });
        }
        updateURL();
          ...
          if (Howler._volume < 1 && 0 === lockEv.length) {
            ...
            lastAudioArr.forEach(function (key, index, array) {
              key.fade(1, 0, 10, lastPlayingArr[index]),
                counterLastAudio++,
                counterLastAudio === array.length && callback();
            });
          }
          0 === lockEv.length &&
            (lastAudioArr.forEach(function (key, index) {
              key.fade(1, 0, 10, lastPlayingArr[index]);
            }),
            (lastAudioArr = []),
            (lastPlayingArr = []));
      ...
      function soundReduction() {
        lastAudioArr.length >= 2 && Howler.volume(0.8);
      }
        ...
        $(".key > span").each(function () {
          ...
          $(".key > span")
            .on("mousedown", function (e) {
              ...
              (lastPlaying = notes[noteClick].play()),
                new Date() - counter > 150 && stopLastAudio(),
                key.addClass("active"),
                lastAudioArr.push(notes[noteClick]),
                lastPlayingArr.push(lastPlaying),
                (counter = new Date()),
                markTrigger || markNotes(key);
            })
      ...
      function playMarkedNotes() {
        var allMarkedNotes = document.querySelectorAll(".marked > span"),
          isActiveMarkedNotes = [];
        allMarkedNotes.length >= 2 ? Howler.volume(0.8) : Howler.volume(1),
          allMarkedNotes.forEach(function (key) {
            var markedNote = notes[key.parentNode.dataset.note],
              $key = $('[data-note="' + key.parentNode.dataset.note + '"]');
            isActiveMarkedNotes.push($key),
              (lastPlaying = markedNote.play()),
              $key.addClass("active"),
              (lockEventMarkedPlay = !0),
              lastAudioArr.push(markedNote),
              lastPlayingArr.push(lastPlaying),
              setTimeout(function () {
      ...
      function hideNotes() {
        ...
        button.innerText =
          button.innerText == buttonNames.showHideNotes.show
            ? buttonNames.showHideNotes.hide
            : buttonNames.showHideNotes.show;
        ...
        if (query !== url && "" !== query) {
          for (i = 0; i < pairs.length; i++)
            (nv = pairs[i].split("=", 2)),
              ...
              parameters.hasOwnProperty(n) || (parameters[n] = []),
              parameters[n].push(2 === nv.length ? v : null),
              elements.push(n);
          return elements;
        }
      }
      ...
      var buttonNames = PIANO_TRANSLATION.buttonNames;
      $(".mark").text(buttonNames.markNotes),
        $(".playAll").text(buttonNames.playNotes),
        $(".btn-reset").text(buttonNames.reset),
        $(".hideNotes").text(buttonNames.showHideNotes.hide);
      var isTouched = !1,
        keyNames = PIANO_TRANSLATION.noteNames;
      $("[data-note]").each(function () {
        var noteHTML = $(this)[0].dataset.note.slice(1);
        $(this).html(
          "<i></i><span class='note'>" + keyNames[noteHTML] + "</span>"
        );
      });
      for (
        var notes = {
            "1c": new Howl({ src: ["../library/piano_react/note/28.mp3"] }),
            "1cis": new Howl({ src: ["../library/piano_react/note/29.mp3"] }),
            "1d": new Howl({ src: ["../library/piano_react/note/30.mp3"] }),
            "1dis": new Howl({ src: ["../library/piano_react/note/31.mp3"] }),
            "1e": new Howl({ src: ["../library/piano_react/note/32.mp3"] }),
            "1f": new Howl({ src: ["../library/piano_react/note/33.mp3"] }),
            "1fis": new Howl({ src: ["../library/piano_react/note/34.mp3"] }),
            "1g": new Howl({ src: ["../library/piano_react/note/35.mp3"] }),
            "1gis": new Howl({ src: ["../library/piano_react/note/36.mp3"] }),
            "2a": new Howl({ src: ["../library/piano_react/note/37.mp3"] }),
            "2ais": new Howl({ src: ["../library/piano_react/note/38.mp3"] }),
            "2b": new Howl({ src: ["../library/piano_react/note/39.mp3"] }),
            "2c": new Howl({ src: ["../library/piano_react/note/40.mp3"] }),
            "2cis": new Howl({ src: ["../library/piano_react/note/41.mp3"] }),
            "2d": new Howl({ src: ["../library/piano_react/note/42.mp3"] }),
            "2dis": new Howl({ src: ["../library/piano_react/note/43.mp3"] }),
            "2e": new Howl({ src: ["../library/piano_react/note/44.mp3"] }),
            "2f": new Howl({ src: ["../library/piano_react/note/45.mp3"] }),
            "2fis": new Howl({ src: ["../library/piano_react/note/46.mp3"] }),
            "2g": new Howl({ src: ["../library/piano_react/note/47.mp3"] }),
            "2gis": new Howl({ src: ["../library/piano_react/note/48.mp3"] }),
            "3a": new Howl({ src: ["../library/piano_react/note/49.mp3"] }),
            "3ais": new Howl({ src: ["../library/piano_react/note/50.mp3"] }),
            "3b": new Howl({ src: ["../library/piano_react/note/51.mp3"] }),
            "3c": new Howl({ src: ["../library/piano_react/note/52.mp3"] }),
            "3cis": new Howl({ src: ["../library/piano_react/note/53.mp3"] }),
            "3d": new Howl({ src: ["../library/piano_react/note/54.mp3"] }),
            "3dis": new Howl({ src: ["../library/piano_react/note/55.mp3"] }),
            "3e": new Howl({ src: ["../library/piano_react/note/56.mp3"] }),
            "3f": new Howl({ src: ["../library/piano_react/note/57.mp3"] }),
            "3fis": new Howl({ src: ["../library/piano_react/note/58.mp3"] }),
            "3g": new Howl({ src: ["../library/piano_react/note/59.mp3"] }),
            "3gis": new Howl({ src: ["../library/piano_react/note/60.mp3"] }),
            "4a": new Howl({ src: ["../library/piano_react/note/61.mp3"] }),
            "4ais": new Howl({ src: ["../library/piano_react/note/62.mp3"] }),
            "4b": new Howl({ src: ["../library/piano_react/note/63.mp3"] }),
          },
          lockEvent = {},
          i = 0;
        256 > i;
      ...
      $(window).bind("keydown keyup", function (ev) {
        ...
        "keydown" == ev.type && note
          ? lockEvent[keyNo] ||
            ((lastPlaying = note.play()),
            new Date() - counter > 100 ? stopLastAudio() : soundReduction(),
            (counter = new Date()),
            lastAudioArr.push(note),
            lastPlayingArr.push(lastPlaying),
            (lockEvent[keyNo] = !0),
            $key.addClass("active"),
          ...
          : "keyup" == ev.type &&
            ...
            isNotesMarked()),
          "keydown" == ev.type && 32 === ev.keyCode
            ? (ev.preventDefault(),
              spacebarPressed || (playMarkedNotes(), (spacebarPressed = !0)))
            : "keyup" == ev.type && 32 === ev.keyCode && (spacebarPressed = !1);
      });
      var markTrigger = !0,
        collectAllNotes = [],
        collectNotesToArr = [],
      ...
      markBtn.addEventListener("click", markKeys),
        ...
        playOnClick();
      var pianoURL = PIANO_TRANSLATION.url;
      (window.onload = codeAddress),
        window.addEventListener("popstate", backButtonFunctionalityURL, !1);
    })();
