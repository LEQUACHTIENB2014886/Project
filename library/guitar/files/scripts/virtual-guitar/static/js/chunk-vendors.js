          ...
          var l = {},
            ...
            h = function () {
              if (a.usingWebAudio) {
                ...
                var isIOS = /iP(hone|od|ad)/.test(a._navigator && a._navigator.platform);
                var iosVersion = a._navigator && a._navigator.appVersion.match(/OS (\d+)_(\d+)_?(\d+)?/);
                var iOSMajorVersion = iosVersion ? parseInt(iosVersion[1], 10) : null;
            
                if (isIOS && iOSMajorVersion && iOSMajorVersion < 9) {
                  var isSafari = /safari/.test((a._navigator && a._navigator.userAgent.toLowerCase()));
                  if (a._navigator && !isSafari) {
                    a.usingWebAudio = false;
                  }
                }
            
                if (a.usingWebAudio) {
                  a.masterGain = a.ctx.createGain ? a.ctx.createGain() : a.ctx.createGainNode();
